   ...
   
  {
    ...
    for(int i=0;i<h;i++)
    {
      for(int j=0;j<w;j++)
      {
        ...
        // #DQF - explicit cast to eliminate compiler warning
        float distValue = (float)sqrt(xValue * xValue + yValue * yValue) + turbPower * turbulence(j, i, turbSize, iturbSize) / 256.0f;
        //
