...
IPlugQuartz::IPlugQuartz(IPlugInstanceInfo instanceInfo)
...
IPlugQuartz::~IPlugQuartz() {}

void IPlugQuartz::ProcessDoubleReplacing(double** inputs, double** outputs, int nFrames)
{
...
void IPlugQuartz::OnParamChange(int paramIdx)
{
	TRACE;
...
bool IPlugQuartz::SerializeState(ByteChunk* pChunk)
{
	TRACE;
	IMutexLock lock(this);
...
int IPlugQuartz::UnserializeState(ByteChunk* pChunk, int startPos)
{
	TRACE;
	IMutexLock lock(this);
...
bool IPlugQuartz::CompareState(const unsigned char* incomingState, int startPos)
{
	bool isEqual = true;
...
void IPlugQuartz::PresetsChangedByHost()
{
	TRACE;
	IMutexLock lock(this);
...
void IPlugQuartz::BeginMIDILearn(int paramIdx1, int paramIdx2, int x, int y)
{
	if (GetAPI() == kAPIVST3)
	{
...
void IPlugQuartz::ProcessMidiMsg(IMidiMsg* pMsg)
{
#ifdef TRACER_BUILD
	pMsg->LogMsg();
#endif
...
void IPlugQuartz::HandleMidiControlChange(IMidiMsg* pMsg)
{
	const IMidiMsg::EControlChangeMsg cc = pMsg->ControlChangeIdx();
	for (int i = 0; i < kNumParams; ++i)
	{
...
void IPlugQuartz::SetControlChangeForParam(const IMidiMsg::EControlChangeMsg cc, const int paramIdx)
{
...
void IPlugQuartz::BroadcastParamChange(const int paramIdx)
{
...
bool IPlugQuartz::HostRequestingAboutBox()
{
#if SA_API
#ifdef OS_WIN
